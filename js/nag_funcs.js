...
/* ── Headertable fix ──────────────────────────────────────────────────────
 * Hide the info/links first cell of any headertable on the page.
 * We target only cells that have NO class attribute — that is the outer
 * info cell.  Named cells (hostTotalsUP, serviceTotals, etc.) are skipped. */
$(document).ready(function() {
	$('table.headertable').each(function() {
		/* Use .children() — direct descendants only — so nested wrapper
		   tables inside the totals cells are never touched. */
		$(this).children('tbody').children('tr')
			.children('td:first-child').hide();
	});
});
